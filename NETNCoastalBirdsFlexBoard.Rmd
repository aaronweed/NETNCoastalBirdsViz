...
amoy_MP<-subset(amoy_MP, Survey_Year == 2019)

colors<-colorNumeric(palette="viridis",domain=amoy_MP$Pair_Count, reverse = TRUE)
...
leaflet() %>%
    ...
    addLayersControl(
      ...
      options = layersControlOptions(collapsed = FALSE),
      position = 'bottomleft') %>% 
   addCircleMarkers(data= amoy_MP,lat= ~lat, lng=~lng, color= colors(amoy_MP$Pair_Count), fillColor =colors(amoy_MP$Pair_Count), radius= ~amoy_MP$Pair_Count*1.5,  opacity=01,fill=TRUE,fillOpacity = 0.8) %>% 
  addLegend(pal=colors,
                values=amoy_MP$Pair_Count[amoy_MP$Survey_Year == 2019],opacity=1, 
